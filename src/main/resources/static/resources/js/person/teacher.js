...
function initTable(){
	$("#Table").width("100%").dataTable({
		"columns":[
		 ...
		 ajax: {
		     url:webpath+'/Teacher/selectPage',
		     "type": 'POST',
...
//	form.cleanValidator(formObj);
	layer.open({
		type: 1,
        title:'<i class="iconfont">&#xe641;</i>&nbsp;新增教师信息',
        area: ['300px', '300px'],
        content: $("#add"),
        btn: ['确定','取消'],
...
function update(id){
	$.ajax({
		...
		success:function(data){
			...
			layer.open({
				type: 1,
		        title:'<i class="iconfont">&#xe633;</i>&nbsp;修改教师信息',
		        area: ['300px', '300px'],
		        content: $("#update"),
		        btn: ['确定','取消'],
		        btn1: function(index, layero){//确定按钮回调
